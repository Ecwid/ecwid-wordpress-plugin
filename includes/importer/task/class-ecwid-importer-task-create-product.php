...
class Ecwid_Importer_Task_Create_Product extends Ecwid_Importer_Task_Product_Base
{
	...
	public function get_batch_request_item( Ecwid_Importer $exporter, array $product_data ) {
		...
		if ( $exporter->get_setting( Ecwid_Importer::SETTING_UPDATE_BY_SKU ) ) {
			...
			if ( $products->total > 0 ) {
				...
				$result = $api->batch_update_product( $data, $ecwid_id );
		...
		if ( !$result ) {
			$result = $api->batch_create_product( $data );
