...
class Ecwid_Importer {
	...
	const OPTIONS_SEPARATE_IMAGE_LOADING = 'ec_woo_import_separate_image_loading';
		...
		do {
			...
			if ( ! isset( $status['plan_limit'] )
				...
				if ( $result['status'] == 'error' ) {
					...
					if ( ! is_wp_error( $error_data ) && is_array( $error_data ) && isset( $error_data['response']['code'] ) && $error_data['response']['code'] == 402 ) {
					...
					if ( ! isset( $progress['error_messages'][ $task_data['type'] ][ $message ] ) ) {
						$progress['error_messages'][ $task_data['type'] ][ $message ] = array();
					}
